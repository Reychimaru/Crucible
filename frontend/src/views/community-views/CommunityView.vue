    ...
    <article class="altContainer flexcolumn">
                ...
                <div class="flexcolumn">
                    <div class="profileMain flexrow-h-end">
                        ...
                        <div class="profileButtons flexrow-h-end">
                            <RouterLink to="/community/:id/dashboard">
                                <button class="buttonText" v-if="isMod || isAdmin">Manage</button>
                            </RouterLink>
                            <button class="buttonText">Joined</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section class="profileFooter flexcolumn">
            <div class="flexcolumn-h-start">
                <div class="flexcolumn-h-start">
                    <h2 class="profileNickname">{{ name }}</h2>
                    <h3 class="username" v-if="!private">Public Community</h3>
                    <h3 class="username" v-if="private">Private Community</h3>
                </div>
                <h3 class="bio">{{ bio }}</h3>
                <p class="profileWebsite"><a :href="`http://${website}`">{{ website }}</a></p>
                <p class="profileStats">
                    <RouterLink to="/community/id/members">{{ members }} Members</RouterLink> | {{ posts.length }} Posts
                </p>
            </div>
        </section>
        <nav class="flexcolumn">
            <div class="flexrow">
                <button v-if="!private && !isMember" class="buttonText">Request Access</button>
            </div>
            <input type="search" v-model="searchQuery" :placeholder="`Search posts in ${community}`">
        </nav>
    </article>
...
export default {
    ...
    data() {
        return {
            cover: "/public/asset/CommunityCover.jpg",
